# Third-Party Licenses

This file is generated from the resolved Cargo dependency graph for `rom-weaver` using normal/build dependencies (`cargo tree --workspace --edges normal,build`).

## Inventory

| Crate | Version | License Expression | Source |
|---|---|---|---|
| `adler2` | `2.0.1` | `0BSD OR MIT OR Apache-2.0` | `crates.io` |
| `aes` | `0.9.0` | `MIT OR Apache-2.0` | `crates.io` |
| `aho-corasick` | `1.1.4` | `Unlicense OR MIT` | `crates.io` |
| `allocator-api2` | `0.2.21` | `MIT OR Apache-2.0` | `crates.io` |
| `ansi_term` | `0.12.1` | `MIT` | `crates.io` |
| `anstream` | `1.0.0` | `MIT OR Apache-2.0` | `crates.io` |
| `anstyle` | `1.0.14` | `MIT OR Apache-2.0` | `crates.io` |
| `anstyle-parse` | `1.0.0` | `MIT OR Apache-2.0` | `crates.io` |
| `anstyle-query` | `1.1.5` | `MIT OR Apache-2.0` | `crates.io` |
| `arrayref` | `0.3.9` | `BSD-2-Clause` | `crates.io` |
| `arrayvec` | `0.7.6` | `MIT OR Apache-2.0` | `crates.io` |
| `atty` | `0.2.14` | `MIT` | `crates.io` |
| `autocfg` | `0.1.8` | `Apache-2.0 OR MIT` | `crates.io` |
| `autocfg` | `1.5.0` | `Apache-2.0 OR MIT` | `crates.io` |
| `base16ct` | `1.0.0` | `Apache-2.0 OR MIT` | `crates.io` |
| `bincode` | `1.3.3` | `MIT` | `crates.io` |
| `bindgen` | `0.52.0` | `BSD-3-Clause` | `crates.io` |
| `bit-set` | `0.6.0` | `MIT/Apache-2.0` | `crates.io` |
| `bit-vec` | `0.7.0` | `MIT/Apache-2.0` | `crates.io` |
| `bitflags` | `1.3.2` | `MIT/Apache-2.0` | `crates.io` |
| `bitflags` | `2.11.1` | `MIT OR Apache-2.0` | `crates.io` |
| `blake3` | `1.8.5` | `CC0-1.0 OR Apache-2.0 OR Apache-2.0 WITH LLVM-exception` | `crates.io` |
| `block-buffer` | `0.10.4` | `MIT OR Apache-2.0` | `crates.io` |
| `block-buffer` | `0.12.0` | `MIT OR Apache-2.0` | `crates.io` |
| `block-padding` | `0.4.2` | `MIT OR Apache-2.0` | `crates.io` |
| `byteorder` | `1.5.0` | `Unlicense OR MIT` | `crates.io` |
| `bytes` | `1.11.1` | `MIT` | `crates.io` |
| `bzip2` | `0.4.4` | `MIT/Apache-2.0` | `crates.io` |
| `bzip2` | `0.6.1` | `MIT OR Apache-2.0` | `crates.io` |
| `bzip2-sys` | `0.1.13+1.0.8` | `MIT/Apache-2.0` | `crates.io` |
| `cbc` | `0.2.0` | `MIT OR Apache-2.0` | `crates.io` |
| `cc` | `1.2.62` | `MIT OR Apache-2.0` | `crates.io` |
| `cdivsufsort` | `2.0.0` | `MIT` | `crates.io` |
| `cexpr` | `0.3.6` | `Apache-2.0/MIT` | `crates.io` |
| `cfg-if` | `0.1.10` | `MIT/Apache-2.0` | `crates.io` |
| `cfg-if` | `1.0.4` | `MIT OR Apache-2.0` | `crates.io` |
| `cipher` | `0.5.1` | `MIT OR Apache-2.0` | `crates.io` |
| `clang-sys` | `0.28.1` | `Apache-2.0` | `crates.io` |
| `clap` | `2.34.0` | `MIT` | `crates.io` |
| `clap` | `4.6.1` | `MIT OR Apache-2.0` | `crates.io` |
| `clap_builder` | `4.6.0` | `MIT OR Apache-2.0` | `crates.io` |
| `clap_derive` | `4.6.1` | `MIT OR Apache-2.0` | `crates.io` |
| `clap_lex` | `1.1.0` | `MIT OR Apache-2.0` | `crates.io` |
| `colorchoice` | `1.0.5` | `MIT OR Apache-2.0` | `crates.io` |
| `const-oid` | `0.10.2` | `Apache-2.0 OR MIT` | `crates.io` |
| `constant_time_eq` | `0.4.2` | `CC0-1.0 OR MIT-0 OR Apache-2.0` | `crates.io` |
| `cpubits` | `0.1.1` | `MIT OR Apache-2.0` | `crates.io` |
| `cpufeatures` | `0.2.17` | `MIT OR Apache-2.0` | `crates.io` |
| `cpufeatures` | `0.3.0` | `MIT OR Apache-2.0` | `crates.io` |
| `crc` | `3.4.0` | `MIT OR Apache-2.0` | `crates.io` |
| `crc-catalog` | `2.5.0` | `MIT OR Apache-2.0` | `crates.io` |
| `crc16` | `0.4.0` | `MIT` | `crates.io` |
| `crc32c` | `0.6.8` | `Apache-2.0/MIT` | `crates.io` |
| `crc32fast` | `1.5.0` | `MIT OR Apache-2.0` | `crates.io` |
| `crossbeam-deque` | `0.8.6` | `MIT OR Apache-2.0` | `crates.io` |
| `crossbeam-epoch` | `0.9.18` | `MIT OR Apache-2.0` | `crates.io` |
| `crossbeam-utils` | `0.8.21` | `MIT OR Apache-2.0` | `crates.io` |
| `crypto-common` | `0.1.7` | `MIT OR Apache-2.0` | `crates.io` |
| `crypto-common` | `0.2.1` | `MIT OR Apache-2.0` | `crates.io` |
| `deranged` | `0.5.8` | `MIT OR Apache-2.0` | `crates.io` |
| `digest` | `0.10.7` | `MIT OR Apache-2.0` | `crates.io` |
| `digest` | `0.11.3` | `MIT OR Apache-2.0` | `crates.io` |
| `dyn-clone` | `1.0.20` | `MIT OR Apache-2.0` | `crates.io` |
| `either` | `1.15.0` | `MIT OR Apache-2.0` | `crates.io` |
| `encoding_rs` | `0.8.35` | `(Apache-2.0 OR MIT) AND BSD-3-Clause` | `crates.io` |
| `env_logger` | `0.7.1` | `MIT/Apache-2.0` | `crates.io` |
| `equivalent` | `1.0.2` | `Apache-2.0 OR MIT` | `crates.io` |
| `errno` | `0.3.14` | `MIT OR Apache-2.0` | `crates.io` |
| `filetime` | `0.2.29` | `MIT/Apache-2.0` | `crates.io` |
| `filetime_creation` | `0.2.0` | `MIT/Apache-2.0` | `crates.io` |
| `find-msvc-tools` | `0.1.9` | `MIT OR Apache-2.0` | `crates.io` |
| `flate2` | `1.1.9` | `MIT OR Apache-2.0` | `crates.io` |
| `foldhash` | `0.2.0` | `Zlib` | `crates.io` |
| `generic-array` | `0.14.7` | `MIT` | `crates.io` |
| `glob` | `0.3.3` | `MIT OR Apache-2.0` | `crates.io` |
| `hashbrown` | `0.16.1` | `MIT OR Apache-2.0` | `crates.io` |
| `heck` | `0.5.0` | `MIT OR Apache-2.0` | `crates.io` |
| `higher-kinded-types` | `0.2.1` | `Zlib OR MIT OR Apache-2.0` | `crates.io` |
| `humantime` | `1.3.0` | `MIT/Apache-2.0` | `crates.io` |
| `hybrid-array` | `0.4.12` | `MIT OR Apache-2.0` | `crates.io` |
| `inout` | `0.2.2` | `MIT OR Apache-2.0` | `crates.io` |
| `is_terminal_polyfill` | `1.70.2` | `MIT OR Apache-2.0` | `crates.io` |
| `itertools` | `0.14.0` | `MIT OR Apache-2.0` | `crates.io` |
| `itoa` | `1.0.18` | `MIT OR Apache-2.0` | `crates.io` |
| `jobserver` | `0.1.34` | `MIT OR Apache-2.0` | `crates.io` |
| `lazy_static` | `1.5.0` | `MIT OR Apache-2.0` | `crates.io` |
| `lazycell` | `1.3.0` | `MIT/Apache-2.0` | `crates.io` |
| `libbz2-rs-sys` | `0.2.4` | `bzip2-1.0.6` | `crates.io` |
| `libc` | `0.2.186` | `MIT OR Apache-2.0` | `crates.io` |
| `libloading` | `0.5.2` | `ISC` | `crates.io` |
| `liblzma` | `0.4.6` | `MIT OR Apache-2.0` | `crates.io` |
| `liblzma-sys` | `0.4.6` | `MIT OR Apache-2.0` | `crates.io` |
| `log` | `0.4.29` | `MIT OR Apache-2.0` | `crates.io` |
| `lru` | `0.16.4` | `MIT` | `crates.io` |
| `lzma-rust` | `0.1.7` | `Apache-2.0` | `crates.io` |
| `macro_rules_attribute` | `0.2.2` | `Apache-2.0 OR MIT OR Zlib` | `crates.io` |
| `macro_rules_attribute-proc_macro` | `0.2.2` | `Apache-2.0 OR MIT OR Zlib` | `crates.io` |
| `md-5` | `0.11.0` | `MIT OR Apache-2.0` | `crates.io` |
| `memchr` | `2.8.0` | `Unlicense OR MIT` | `crates.io` |
| `memmap2` | `0.9.10` | `MIT OR Apache-2.0` | `crates.io` |
| `miniz_oxide` | `0.8.9` | `MIT OR Zlib OR Apache-2.0` | `crates.io` |
| `never-say-never` | `6.6.666` | `Zlib OR MIT OR Apache-2.0` | `crates.io` |
| `nod` | `2.0.0-alpha.6` | `MIT OR Apache-2.0` | `crates.io` |
| `nom` | `4.2.3` | `MIT` | `crates.io` |
| `nt-time` | `0.8.1` | `Apache-2.0 OR MIT` | `crates.io` |
| `num-conv` | `0.2.1` | `MIT OR Apache-2.0` | `crates.io` |
| `num-traits` | `0.2.19` | `MIT OR Apache-2.0` | `crates.io` |
| `num_cpus` | `1.17.0` | `MIT OR Apache-2.0` | `crates.io` |
| `once_cell` | `1.21.4` | `MIT OR Apache-2.0` | `crates.io` |
| `paste` | `1.0.15` | `MIT OR Apache-2.0` | `crates.io` |
| `peeking_take_while` | `0.1.2` | `Apache-2.0/MIT` | `crates.io` |
| `pin-project-lite` | `0.2.17` | `Apache-2.0 OR MIT` | `crates.io` |
| `pkg-config` | `0.3.33` | `MIT OR Apache-2.0` | `crates.io` |
| `polonius-the-crab` | `0.5.0` | `Zlib OR MIT OR Apache-2.0` | `crates.io` |
| `powerfmt` | `0.2.0` | `MIT OR Apache-2.0` | `crates.io` |
| `proc-macro2` | `1.0.106` | `MIT OR Apache-2.0` | `crates.io` |
| `qbsdiff` | `1.4.4` | `MIT` | `crates.io` |
| `quick-error` | `1.2.3` | `MIT/Apache-2.0` | `crates.io` |
| `quote` | `1.0.45` | `MIT OR Apache-2.0` | `crates.io` |
| `rand` | `0.6.5` | `MIT/Apache-2.0` | `crates.io` |
| `rand_chacha` | `0.1.1` | `MIT/Apache-2.0` | `crates.io` |
| `rand_core` | `0.3.1` | `MIT/Apache-2.0` | `crates.io` |
| `rand_core` | `0.4.2` | `MIT/Apache-2.0` | `crates.io` |
| `rand_hc` | `0.1.0` | `MIT/Apache-2.0` | `crates.io` |
| `rand_isaac` | `0.1.1` | `MIT/Apache-2.0` | `crates.io` |
| `rand_jitter` | `0.1.4` | `MIT OR Apache-2.0` | `crates.io` |
| `rand_os` | `0.1.3` | `MIT/Apache-2.0` | `crates.io` |
| `rand_pcg` | `0.1.2` | `MIT/Apache-2.0` | `crates.io` |
| `rand_xorshift` | `0.1.1` | `MIT/Apache-2.0` | `crates.io` |
| `rayon` | `1.12.0` | `MIT OR Apache-2.0` | `crates.io` |
| `rayon-core` | `1.13.0` | `MIT OR Apache-2.0` | `crates.io` |
| `regex` | `1.12.3` | `MIT OR Apache-2.0` | `crates.io` |
| `regex-automata` | `0.4.14` | `MIT OR Apache-2.0` | `crates.io` |
| `regex-syntax` | `0.8.10` | `MIT OR Apache-2.0` | `crates.io` |
| `rustc-hash` | `1.1.0` | `Apache-2.0/MIT` | `crates.io` |
| `rustc_version` | `0.4.1` | `MIT OR Apache-2.0` | `crates.io` |
| `rustix` | `1.1.4` | `Apache-2.0 WITH LLVM-exception OR Apache-2.0 OR MIT` | `crates.io` |
| `sacabase` | `2.0.0` | `MIT` | `crates.io` |
| `semver` | `1.0.28` | `MIT OR Apache-2.0` | `crates.io` |
| `serde` | `1.0.228` | `MIT OR Apache-2.0` | `crates.io` |
| `serde_core` | `1.0.228` | `MIT OR Apache-2.0` | `crates.io` |
| `serde_derive` | `1.0.228` | `MIT OR Apache-2.0` | `crates.io` |
| `serde_json` | `1.0.149` | `MIT OR Apache-2.0` | `crates.io` |
| `sevenz-rust` | `0.6.1` | `Apache-2.0` | `crates.io` |
| `sha1` | `0.11.0` | `MIT OR Apache-2.0` | `crates.io` |
| `sha2` | `0.10.9` | `MIT OR Apache-2.0` | `crates.io` |
| `shlex` | `0.1.1` | `MIT/Apache-2.0` | `crates.io` |
| `shlex` | `1.3.0` | `MIT OR Apache-2.0` | `crates.io` |
| `simd-adler32` | `0.3.9` | `MIT` | `crates.io` |
| `simple_moving_average` | `1.0.2` | `MIT` | `crates.io` |
| `strsim` | `0.11.1` | `MIT` | `crates.io` |
| `strsim` | `0.8.0` | `MIT` | `crates.io` |
| `suffix_array` | `0.5.0` | `MIT` | `crates.io` |
| `syn` | `2.0.117` | `MIT OR Apache-2.0` | `crates.io` |
| `tar` | `0.4.45` | `MIT OR Apache-2.0` | `crates.io` |
| `termcolor` | `1.4.1` | `Unlicense OR MIT` | `crates.io` |
| `textwrap` | `0.11.0` | `MIT` | `crates.io` |
| `thiserror` | `1.0.69` | `MIT OR Apache-2.0` | `crates.io` |
| `thiserror` | `2.0.18` | `MIT OR Apache-2.0` | `crates.io` |
| `thiserror-impl` | `1.0.69` | `MIT OR Apache-2.0` | `crates.io` |
| `thiserror-impl` | `2.0.18` | `MIT OR Apache-2.0` | `crates.io` |
| `threadpool` | `1.8.1` | `MIT/Apache-2.0` | `crates.io` |
| `time` | `0.3.47` | `MIT OR Apache-2.0` | `crates.io` |
| `time-core` | `0.1.8` | `MIT OR Apache-2.0` | `crates.io` |
| `time-macros` | `0.2.27` | `MIT OR Apache-2.0` | `crates.io` |
| `tracing` | `0.1.44` | `MIT` | `crates.io` |
| `tracing-attributes` | `0.1.31` | `MIT` | `crates.io` |
| `tracing-core` | `0.1.36` | `MIT` | `crates.io` |
| `typenum` | `1.20.0` | `MIT OR Apache-2.0` | `crates.io` |
| `unicode-ident` | `1.0.24` | `(MIT OR Apache-2.0) AND Unicode-3.0` | `crates.io` |
| `unicode-width` | `0.1.14` | `MIT OR Apache-2.0` | `crates.io` |
| `utf8parse` | `0.2.2` | `Apache-2.0 OR MIT` | `crates.io` |
| `vec_map` | `0.8.2` | `MIT/Apache-2.0` | `crates.io` |
| `version_check` | `0.1.5` | `MIT/Apache-2.0` | `crates.io` |
| `version_check` | `0.9.5` | `MIT/Apache-2.0` | `crates.io` |
| `which` | `3.1.1` | `MIT` | `crates.io` |
| `widestring` | `1.2.1` | `MIT OR Apache-2.0` | `crates.io` |
| `xattr` | `1.6.1` | `MIT OR Apache-2.0` | `crates.io` |
| `xxhash-rust` | `0.8.15` | `BSL-1.0` | `crates.io` |
| `zerocopy` | `0.8.48` | `BSD-2-Clause OR Apache-2.0 OR MIT` | `crates.io` |
| `zerocopy-derive` | `0.8.48` | `BSD-2-Clause OR Apache-2.0 OR MIT` | `crates.io` |
| `zip` | `0.6.6` | `MIT` | `crates.io` |
| `zmij` | `1.0.21` | `MIT` | `crates.io` |
| `zstd` | `0.11.2+zstd.1.5.2` | `MIT` | `crates.io` |
| `zstd` | `0.13.3` | `MIT` | `crates.io` |
| `zstd-safe` | `5.0.2+zstd.1.5.2` | `MIT/Apache-2.0` | `crates.io` |
| `zstd-safe` | `7.2.4` | `MIT OR Apache-2.0` | `crates.io` |
| `zstd-seekable` | `0.1.23` | `BSD-3-Clause` | `crates.io` |
| `zstd-sys` | `2.0.16+zstd.1.5.7` | `MIT/Apache-2.0` | `crates.io` |

## Notes

- This list excludes local workspace crates (`rom-weaver-*`).
- This list is resolved from current Cargo metadata and may change when dependency versions change.
