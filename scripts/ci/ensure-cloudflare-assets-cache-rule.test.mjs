...
test("updates both managed rules without disturbing unrelated rules", async () => {
        ...
        options.method === "PUT"
          ...
          : {
              ...
              result: {
                rules: [
                  unrelatedRule,
                  { ...cacheRule(), action_parameters: { cache: true } },
                  {
  ...
  assert.equal(await ensureCacheRule({ zoneId: "zone", token: "token", fetchImpl }), "installed");
  assert.deepEqual(JSON.parse(requests[1].options.body).rules, [
    unrelatedRule,
    cacheRule(),
    documentCacheRule(),
  ]);
