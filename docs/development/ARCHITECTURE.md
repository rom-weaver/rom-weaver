...
The browser uses the engine's checksum variants to resolve headers without re-hashing. A `remove-header` row carries `transforms.removeHeader`, including `strippedBytes`, `retainOnOutput`, and the extension pair. When `lib/workflow/apply-header-resolution.ts` proves a headerless basis, staging strips the header and sends `removeHeader`. Otherwise it sends `auto`, including for the first patch, so the engine can infer the basis. The download filename follows the path the engine emits, including any extension change.

`resolveApplyHeaderMode` returns `decided: false` when the browser cannot prove the basis. The header control shows `auto` through `formatHeaderAutoLabel` because the engine must resolve the result during apply.
...
Bundles describe ordered patch workflows. The shared schema and parser live in `crates/rom-weaver-cli/src/bundle_schema.rs` and `bundle_parse.rs`. Validation failures use stable `bundle.*` codes on `RomWeaverError`. [What a bundle is](../explanation/bundles.md) explains the user model; the [CLI reference](../reference/cli.md#bundles) and [JSON Schema](../rom-weaver-bundle-v2.schema.json) own the public fields and options.

**Chain checks.** Version 2 requires `patchBasis` for the shared authored basis, with per-entry `basis` exceptions. `auto` remains the default, and version 1 retains automatic inference. `rom.checks` describes the input and `output.checks` the full-chain result. `checkStates` stores named expected states; `checksRef`, `inputChecksRef`, and `outputChecksRef` share their values. Equal values from unrelated states remain separate. A ROM entry can contain checks and a name without a source; the applying user supplies the bytes. A name mismatch warns, while checksum and size requirements remain strict.

**Execution targets.** A patch's `target` scopes its accumulated chain to a ROM member or generated-output member. Its `input` instead selects exact bytes and retains a fixed producer dependency. Filtering optional patches changes the accumulated chain within each target. It does not change a named dependency. Member extraction reuses the shared input resolver. Authored checksum requirements remain separate from execution selection.
...
- **Workers only.** The OPFS runtime requires a Dedicated Worker because synchronous OPFS access handles are unavailable on the main thread. `packages/rom-weaver-webapp/src/workers/` hosts the worker entrypoints; the protocol types live in its `protocol/` directory.
...
- **Workers share one runtime chunk.** During production builds, the `rom-weaver-share-worker-runtime-chunks` plugin in `packages/rom-weaver-webapp/scripts/vite-config/worker-chunks.mjs` emits `?worker&url` imports as entries in the main build graph. One code-splitting pass then covers the app and workers. The `wasm-runtime` group selects modules reachable only from worker entries. It keeps `includeDependenciesRecursively` off to avoid moving modules used by the document entry into the worker runtime chunk. For the same reason, `src/wasm/index.ts` re-exports `rom-weaver-browser-opfs-api.ts` as types only. Development uses Vite's worker handling because the plugin has `apply: "build"`.
...
- **Localization.** UI chrome strings live in the `ui.*` namespace of `packages/rom-weaver-webapp/src/presentation/localization/catalog.ts` (en/es/de) and are consumed via `useUiLocalizer()` (settings `language` → `createBrowserLocalizer`). Plural ids end in `.one`/`.other` and resolve via `Localizer.messageCount`. The translations themselves live in the `.po` files under `localization/locales/`, which are the source of truth; the `.ts` catalogs beside them are compiled output, untracked, and rebuilt by a Vite plugin on every dev, build, preview, and vitest run. Only the English catalog is in the main bundle; every other locale is its own chunk, fetched the first time the locale resolves. The boot gate in `webapp.ts` preloads the active locale so the first render is translated, and `useUiLocalizer()` re-issues the localizer when a catalog lands. A test that reads translated text synchronously calls `preloadCatalog(locale)` first.
