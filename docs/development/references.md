...
| Header handling | Flips has legacy SMC/SFC 512-byte header removal unless exact handling is requested. MultiPatch inherits that behavior through Flips. | The BPS handler treats inputs as exact byte streams. The CLI applies its separate header-resolution policy before calling the handler; see [Architecture](ARCHITECTURE.md#patch-apply-rom-copier-headers). |
