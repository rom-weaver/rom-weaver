...
| `ROM_WEAVER_7Z_MEM_BUDGET_MB` | u64 (MiB) | physical RAM / 2 (1 GiB wasm / 2 GiB native fallback) | `crates/rom-weaver-containers/src/handlers/sevenz.rs`; `crates/rom-weaver-containers/src/libarchive/memlimit.rs` | Memory budget that caps the LZMA2 multi-thread count for 7z create and the combined live allocation of all LZMA/LZMA2 decoders for 7z read. Readers use half the budget with a 65 MiB minimum. Invalid text is ignored. |
