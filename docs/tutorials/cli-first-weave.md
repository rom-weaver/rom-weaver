...
No internet, or running your own deployment? The same files are generated by
`packages/rom-weaver-webapp/scripts/first-sample-assets.mjs`, and every
deployment serves them at the same relative paths, so swap `rom-weaver.com` for
your own host in the commands below.
