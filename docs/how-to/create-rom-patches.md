...
2. Drop both files onto **0x01 Inputs**, or choose **Add files** and select
   them. If more ROM candidates are available than Create can use, choose the
   Original and Modified files from the list. Extra files are not staged after
   you choose. If both files appear to be the same ROM, confirm before using
   them as a pair. Add patches on **Apply** instead.
