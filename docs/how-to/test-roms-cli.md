...
Native testing initially supports NES ROMs on Linux x86-64 with GNU libc 2.39 or later. It uses the packaged RetroArch and FCEUmm runtime. That runtime is built without networking.
...
Install the CLI version that you want to use. Then install its matching runtime:
...
The offline form works before publication when you have a locally built runtime archive. Both forms verify the archive, manifest, RetroArch binary, and FCEUmm core before installation.
...
The packaged runtime has no networking support. A custom `--runtime-dir` can name a different executable. rom-weaver does not place that executable in an operating-system network sandbox.
...
The input can be an archive. Use `--select` when the archive contains more than one candidate:
