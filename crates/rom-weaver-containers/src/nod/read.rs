    ...
    T: Read + Seek + Send,
{
    ...
    fn lock(&self) -> io::Result<std::sync::MutexGuard<'_, T>> {
        self.0
            .lock()
            .map_err(|_| io::Error::other("NonCloneableStream mutex poisoned"))
    }
}
...
impl dyn PartitionReader + '_ {
    ...
    /// };
    ///
    /// fn main() -> rom_weaver_containers::nod::Result<()> {
...
impl PartitionMeta {
    ...
    pub fn dol_header(&self) -> &DolHeader {
        DolHeader::ref_from_prefix(&self.raw_dol)
            .expect("Invalid DOL alignment")
            .0
    }
    ...
    pub fn tmd_header(&self) -> Option<&TmdHeader> {
        ...
        Some(
            TmdHeader::ref_from_prefix(raw_tmd)
                .expect("Invalid TMD alignment")
                .0,
        )
    }
