...
impl Ticket {
    ...
    pub fn decrypt_title_key(&self) -> Result<KeyBytes> {
        ...
        let cert_issuer_ticket = CStr::from_bytes_until_nul(&self.sig_issuer)
            .ok()
        ...
        let common_keys = match cert_issuer_ticket {
            ...
            None => {
                return Err(Error::DiscFormat(
                    "failed to parse certificate issuer".to_string(),
                ));
            }
        };
        let common_key = common_keys
            .get(self.common_key_idx as usize)
            .ok_or(Error::DiscFormat(format!(
    ...
    fn fill_buf(&mut self) -> io::Result<&[u8]> {
        let (part_sector, sector_offset) = if self.partition.has_hashes {
            ...
            (
                ...
                (self.pos % SECTOR_SIZE as u64) as usize,
            )
        };
...
impl PartitionReader for PartitionReaderWii {
    ...
    fn meta(&mut self) -> Result<PartitionMeta> {
        ...
        meta.raw_cert_chain = if self.partition.header.cert_chain_size() != 0 {
            reader
                .seek(SeekFrom::Start(
                    offset + self.partition.header.cert_chain_off(),
                ))
        ...
        meta.raw_h3_table = if self.partition.has_hashes {
            reader
                .seek(SeekFrom::Start(
                    offset + self.partition.header.h3_table_off(),
                ))
