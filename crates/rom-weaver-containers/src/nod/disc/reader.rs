...
impl DiscReader {
    pub fn new(inner: Box<dyn BlockReader>, options: &DiscOptions) -> Result<Self> {
        ...
        let disc_data = if disc_header.is_wii() {
            ...
            // Read region info
            reader
                .seek(SeekFrom::Start(REGION_OFFSET))
                .context("Seeking to region info")?;
        ...
        let size = io
            .meta()
            .disc_size
            .unwrap_or_else(|| guess_disc_size(partitions));
        ...
        match &self.disc_data {
            DiscReaderData::GameCube { .. } => {
                if index == 0 {
                    ...
                    Err(Error::DiscFormat(
                        "GameCube discs only have one partition".to_string(),
                    ))
                }
            }
        ...
        match &self.disc_data {
            DiscReaderData::GameCube { .. } => {
                if kind == PartitionKind::Data {
                    ...
                    Err(Error::DiscFormat(
                        "GameCube discs only have a data partition".to_string(),
                    ))
                }
            }
            DiscReaderData::Wii { partitions, .. } => {
    ...
    pub fn load_sector_group(
        ...
        let (request, max_groups) = if let Some(partition) = self
            .orig_partitions()
            .iter()
            .find(|part| part.data_contains_sector(abs_sector))
        {
        ...
        if pos < size_of::<DiscHeader>() as u64 {
            if let Some(alt_disc_header) = &self.alt_disc_header {
                return Ok(Bytes::copy_from_slice(
                    &alt_disc_header.as_bytes()[pos as usize..],
                ));
            }
        }
...
fn read_partition_info(
    ...
    reader
        .seek(SeekFrom::Start(WII_PART_GROUP_OFF))
        .context("Seeking to partition groups")?;
