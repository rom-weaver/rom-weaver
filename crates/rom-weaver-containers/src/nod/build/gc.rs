...
impl GCPartitionBuilder {
    ...
    pub fn build(
        ...
        Ok(GCPartitionWriter::new(
            write_info,
            disc_size,
            junk_id,
            self.disc_header.disc_num,
        ))
    }
}
...
impl GCPartitionLayout {
    ...
    fn layout_files(&mut self) -> Result<Vec<WriteInfo>> {
        ...
        if self.boot_header.user_size.get() == 0 {
            ...
            self.boot_header
                .user_size
                .set((user_end - user_offset) as u32);
        }
    ...
    fn junk_id(&self) -> [u8; 4] {
        self.junk_id
            .unwrap_or_else(|| *array_ref![self.disc_header.game_id, 0, 4])
    }
}
...
impl GCPartitionWriter {
    ...
    where
        W: Write + ?Sized,
    {
        let mut out = WriteCursor {
            inner: out,
            position: 0,
        };
        let mut lfg = LaggedFibonacci::default();
        for info in &self.write_info {
            out.write_zeroes_until(info.offset)
        ...
        out.write_zeroes_until(self.disc_size)
            .context("Writing end of file")?;
        ...
        while !buf.is_empty() {
            ...
            let read = if pos < info.offset {
                ...
                match &info.kind {
                    WriteKind::File(name) => {
                        self.file_callback
                            .read_file(&mut buf[..read], name, pos - info.offset)?;
                    }
