            ...
            Format::Iso => {
                if options.compression != Compression::None {
                    return Err(Error::Other(
                        "ISO/GCM does not support compression".to_string(),
                    ));
                }
                Box::new(reader)
            }
            Format::Ciso => crate::nod::io::ciso::DiscWriterCISO::new(reader, &options)?,
            #[cfg(feature = "compress-zlib")]
            Format::Gcz => crate::nod::io::gcz::DiscWriterGCZ::new(reader, &options)?,
            Format::Tgc => crate::nod::io::tgc::DiscWriterTGC::new(reader, &options)?,
            Format::Wbfs => crate::nod::io::wbfs::DiscWriterWBFS::new(reader, &options)?,
            Format::Wia | Format::Rvz => crate::nod::io::wia::DiscWriterWIA::new(reader, &options)?,
