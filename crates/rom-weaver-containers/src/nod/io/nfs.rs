...
impl NFSHeader {
    ...
    fn calculate_num_files(&self) -> u32 {
        let sector_count = self
            .lba_ranges()
            .iter()
            .fold(0u32, |acc, range| acc + range.num_sectors.get());
...
impl BlockReaderNFS {
    pub fn load_files(&mut self, directory: &Path) -> Result<()> {
        {
            // Load key file
            let primary_key_path = get_path(
                directory,
                ["..", "code", "htk.bin"].iter().collect::<PathBuf>(),
            );
...

        {
            ...
            // Calculate sizes
            let num_sectors = header
                .lba_ranges()
                .iter()
                .map(|range| range.num_sectors.get())
                .sum::<u32>();
