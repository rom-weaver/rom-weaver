...
pub fn new(mut stream: Box<dyn DiscStream>) -> Result<Box<dyn BlockReader>> {
    let io: Box<dyn BlockReader> =
        match detect_stream(stream.as_mut()).context("Detecting file type")? {
            Some(Format::Iso) => crate::nod::io::iso::BlockReaderISO::new(stream)?,
            Some(Format::Ciso) => crate::nod::io::ciso::BlockReaderCISO::new(stream)?,
            Some(Format::Gcz) => {
                #[cfg(feature = "compress-zlib")]
                {
                    crate::nod::io::gcz::BlockReaderGCZ::new(stream)?
                }
            ...
            Some(Format::Nfs) => {
                return Err(Error::DiscFormat(
                    "NFS requires a filesystem path".to_string(),
                ));
            }
            Some(Format::Wbfs) => crate::nod::io::wbfs::BlockReaderWBFS::new(stream)?,
            Some(Format::Wia | Format::Rvz) => crate::nod::io::wia::BlockReaderWIA::new(stream)?,
            Some(Format::Tgc) => crate::nod::io::tgc::BlockReaderTGC::new(stream)?,
        ...
        match detect_stream(stream.as_mut()).context("Detecting file type")? {
            ...
            Some(Format::Nfs) => match path.parent() {
                Some(parent) if parent.is_dir() => {
                    crate::nod::io::nfs::BlockReaderNFS::new(path.parent().unwrap())?
                }
                _ => {
                    return Err(Error::DiscFormat(
                        "Failed to locate NFS parent directory".to_string(),
                    ));
                }
            },
            Some(Format::Tgc) => crate::nod::io::tgc::BlockReaderTGC::new(stream)?,
...
impl Block {
    ...
    pub(crate) fn partition_data<'a>(
        ...
        let (part_sector, sector_offset) = if partition_has_hashes {
            (
                (pos / SECTOR_DATA_SIZE as u64) as u32,
                (pos % SECTOR_DATA_SIZE as u64) as usize,
            )
        } else {
            (
                (pos / SECTOR_SIZE as u64) as u32,
