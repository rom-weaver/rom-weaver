...
use transaction::{LibarchiveExtractTransaction, ensure_extract_path_has_no_links};

#[cfg(test)]
use transaction::{normalize_confined_extract_root, wasm_extract_transaction_owner_id};

#[cfg(all(test, not(target_family = "wasm")))]
use transaction::install_staged_no_overwrite_with;
...
mod tests {
    ...
    fn sevenz_entries_report_their_real_solid_block() {
        ...
        for (name, bytes) in [("a.bin", b"abc".as_slice()), ("b.bin", b"def".as_slice())] {
            ...
            archive
                .write_data_all(bytes, "write test entry")
