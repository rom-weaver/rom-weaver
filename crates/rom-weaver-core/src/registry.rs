...
pub trait PatchHandler: Send + Sync {
    ...
    /// Read embedded source and target requirements for the `ingest` describe path.
    /// The default calls [`Self::parse`]; overrides MAY omit the action scan and
    /// record count, so success does not guarantee full structural validation.
