...
thread_local int g_rw_mame_chd_compression_level = 0;

int rw_mame_chd_zlib_level()
{
	if (g_rw_mame_chd_compression_level == 0)
		return Z_BEST_COMPRESSION;
	return std::clamp(g_rw_mame_chd_compression_level, 1, 9);
}

int rw_mame_chd_zstd_level()
{
	if (g_rw_mame_chd_compression_level == 0)
		return ZSTD_maxCLevel();
	return std::clamp(g_rw_mame_chd_compression_level, -7, ZSTD_maxCLevel());
}

int rw_mame_chd_lzma_level()
{
	if (g_rw_mame_chd_compression_level == 0)
		return 8;
	return std::clamp(g_rw_mame_chd_compression_level, 0, 9);
}
	...
	: chd_compressor(chd, hunkbytes, lossy)
{
	...
	int zerr = deflateInit2(&m_deflater, rw_mame_chd_zlib_level(), Z_DEFLATED, -MAX_WBITS, 8, Z_DEFAULT_STRATEGY);
...
{
	// reset the compressor
	auto result = ZSTD_initCStream(m_stream, rw_mame_chd_zstd_level());
...
void chd_lzma_compressor::configure_properties(CLzmaEncProps &props, uint32_t hunkbytes)
{
	...
	props.level = rw_mame_chd_lzma_level();
...
extern "C" void rw_mame_chd_set_compression_level(int level)
{
	g_rw_mame_chd_compression_level = level;
}
