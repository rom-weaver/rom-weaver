...
impl CliApp {
    ...
    pub(super) fn plan_dry_run(&self, command: &Commands) -> Option<AppRunOutcome> {
        match command {
            // Save writes render their field-level preview and integrity summary.
            // The shared planner MUST NOT intercept them.
            Commands::Compress(_)
            | Commands::Trim(_)
            | Commands::Save(SaveCommands::Set(_) | SaveCommands::Create(_)) => None,
