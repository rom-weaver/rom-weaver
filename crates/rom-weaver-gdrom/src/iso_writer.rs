...
pub fn plan_iso(entries: &[IsoEntry], start_lba: u32, timestamp: IsoTimestamp) -> Result<IsoPlan> {
    ...
    for (dir_idx, dir) in placed.iter_mut().enumerate() {
        for (ident, lba_slot, size) in &mut dir.files {
            ...
            *lba_slot = next;
            ...
            next += sectors_for(*size as usize);
        }
    }
    ...
    let root_lba = placed[0].lba + start_lba;
    ...
    let pvd = encode_pvd(
        volume_space_size,
        path_table_size,
        l_path_sector + start_lba,
        m_path_sector + start_lba,
        root_lba,
        root_data_len,
        &timestamp,
    );
...
pub fn build_iso(files: &[IsoFile], start_lba: u32, timestamp: IsoTimestamp) -> Result<Vec<u8>> {
    let entries: Vec<IsoEntry> = files
        .iter()
        .map(|f| IsoEntry {
            path: f.path.clone(),
    ...
    F: FnMut(&PlannedFile) -> Result<Vec<u8>>,
{
    ...
    for sector_index in 0..plan.first_file_sector {
        ...
        emit(sink, plan.start_lba + sector_index, &cooked)?;
    }
    ...
    for file in &plan.files {
        ...
        for i in 0..planned_sectors {
            ...
            emit(sink, plan.start_lba + file.lba + i, &cooked)?;
            offset += take;
        }
    }
    tracing::debug!(
...
fn flatten_tree(
    ...
    let mut next_index = root_index + 1;
    ...
    while let Some((node, name, index, parent)) = queue.pop_front() {
        ...
        let mut len = directory_record_len(1) * 2; // . and ..
...
fn encode_directory_extent(
    ...
    let self_lba = dir.lba + start_lba;
    let parent = &placed[dir.parent_index as usize - 1];
    let parent_lba = parent.lba + start_lba;
...
fn encode_path_table(placed: &[PlacedDir], start_lba: u32, big_endian: bool) -> Vec<u8> {
    let mut out = Vec::new();
    for dir in placed {
        let name = &dir.name;
