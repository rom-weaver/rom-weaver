...
import { __createBrowserOpfsRandomAccessFileForTest } from "../../src/wasm/rom-weaver-browser-opfs-api.ts";
...
function ensureReady() {
  ...
  readFile = __createBrowserOpfsRandomAccessFileForTest(
    createMockSyncAccessHandle({ fill: patternByte, size: fixtureBytes }),
  );
...
describe("rom-weaver-wasm OPFS read-cache CPU micro-benchmarks (mock handle)", () => {
  bench(
    "read small <=256KiB within one block (cache-hit path)",
    () => {
      ensureReady();
  ...
  bench(
    "read 512KiB request (>256KiB, bypasses cache)",
    () => {
      ensureReady();
  ...
  bench(
    "write 64KiB then read same region (invalidation path)",
    () => {
      ensureReady();
  ...
  bench(
    "write elsewhere then re-read warmed region (cross-region invalidation)",
    () => {
      ensureReady();
      const readOffset = 0;
