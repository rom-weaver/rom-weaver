...
} from "../../src/wasm/rom-weaver-browser-opfs-api.ts";
...
beforeEach(() => {
  const syncHandle = createMockSyncAccessHandle({ fill: patternByte, size: FILE_BYTES });
...
describe("BrowserOpfsRandomAccessFile read cache correctness", () => {
  ...
  it("tracks lazy allocation without physically growing the OPFS handle", () => {
    const syncHandle = createMockSyncAccessHandle({ fill: patternByte, size: 1024 });
...
function createTestWasiBackingFile(options = {}) {
  ...
  return {
    ...
    truncate() {
      // no-op: mock adapter does not track truncation
    },
    writeAt(offset, source) {
      if (state.closed) return -1;
      state.writes.push({ bytes: source.slice(), offset: Number(offset) });
