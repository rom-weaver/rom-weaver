...
test("browser OPFS source refs register CHD blobs as virtual WASI inputs", async () => {
...
test("browser OPFS RVZ virtual input preserves the original filename leaf", async () => {
  ...
  try {
    ...
    expect(staged.virtual).toBe(true);
    expect(getActiveBrowserVirtualFiles()[0]?.path).toBe("/work/Luigi's Mansion (USA).rvz");
  } finally {
    await staged.cleanup();
  }
  expect(getActiveBrowserVirtualFiles()).toEqual([]);
...
test("browser OPFS source refs register 7z files as virtual WASI inputs", async () => {
