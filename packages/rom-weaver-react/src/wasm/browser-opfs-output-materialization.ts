...
async function replaceScratchBackedEntriesWithOpfsHandles({
  ...
  for (const [name, entry] of entries) {
    ...
    if (entry instanceof wasiShim.Directory) {
      const childHandle = (await directoryHandle.getDirectoryHandle(name, {
        create: true,
  ...
  for (const [name, entry] of entries) {
    if (isRandomAccessFileInodeLike(entry)) {
      if (entry.scratchBacked) {
        const fileHandle = (await directoryHandle.getFileHandle(name, { create: true })) as OpfsFileHandleLike;
