...
type DeviceMemoryRoot = { navigator?: { deviceMemory?: number } };
 ...
 * a fixed ceiling when the engine does not expose it (Firefox/Safari).
 */
export function resolveMemoryCeilingBytes(root: DeviceMemoryRoot | null = globalThis as DeviceMemoryRoot): number {
  ...
  if (!(Number.isFinite(deviceMemoryGib) && deviceMemoryGib > 0)) return FALLBACK_MEMORY_CEILING_BYTES;
  const ceiling = Math.floor(deviceMemoryGib * BYTES_PER_GIB * DEVICE_MEMORY_FRACTION);
  return Math.max(MIN_MEMORY_CEILING_BYTES, Math.min(MAX_MEMORY_CEILING_BYTES, ceiling));
}
