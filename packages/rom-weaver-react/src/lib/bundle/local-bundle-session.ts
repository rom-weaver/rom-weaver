...
const loadLocalBundleSession = async (bundleFile: File, droppedFiles: File[]) => {
  const parse = browserRuntime.bundle?.parse;
  if (!parse) throw new Error("Bundle parsing is not available in this runtime");
  const { result, extractedFiles } = await parse({ fileName: bundleFile.name, source: bundleFile });
