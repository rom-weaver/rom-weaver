...
type WasiFdPreadImport = (fd: number, iovsPtr: number, iovsLen: number, offset: number | bigint, nreadPtr: number) => unknown;
type WasiFdWriteImport = (fd: number, iovsPtr: number, iovsLen: number, nwrittenPtr: number) => unknown;
type WasiFdPwriteImport = (fd: number, iovsPtr: number, iovsLen: number, offset: number | bigint, nwrittenPtr: number) => unknown;
...
function isDirectWasiFileIoStatsRecord(value: unknown): value is DirectWasiFileIoStats {
  ...
  return typeof record.readBytes === 'number'
...
function isBufferedWriteWasiFd(value: unknown): value is BufferedWriteWasiFd {
  ...
  return typeof candidate.pendingWriteBufferLength === 'function'
    && typeof candidate.flushPendingWrite === 'function';
}
