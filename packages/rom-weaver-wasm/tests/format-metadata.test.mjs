...
} from '../src/generated/rom-weaver-format-metadata.ts';
import { getBrowserFormatMatrixMetadataCoverage } from '../src/browser-format-matrix.ts';
...
describe('generated format metadata parity', () => {
  ...
  it('keeps the browser format matrix covering generated registries', () => {
    ...
    expect(sorted(unique([...coverage.containerRoundTripFormats, ...coverage.containerCompressFailureFormats]))).toEqual(
      sorted(ROM_WEAVER_CONTAINER_FORMATS.map((format) => format.name)),
    );

    const createAliases = ROM_WEAVER_CREATE_PATCH_FORMAT_POLICY.aliases;
    const patchFormats = unique(
      ROM_WEAVER_PATCH_FORMATS.map((format) => createAliases[format.name] ?? format.name),
    );
