// The stylesheet, entry module, and webfont are the only render-critical subresources,
// and the parser cannot discover any of them until the document has been fetched and
// parsed. Emitting them as `Link` response headers lets Cloudflare replay them in a 103
// Early Hints response, so the fetches start during server think time instead of after
// HTML parse. Browsers that ignore 103 still honour the header on the document response
// itself, which is earlier than the parser either way.
//
// Every hint uses `rel=preload`, not `rel=modulepreload`, because Cloudflare only replays
...
//
// All three carry `crossorigin` to match how the document fetches them - the stylesheet
// link and the module script have the attribute, and font fetches are always CORS-mode -
// or the browser would fetch each twice.
//
...
//
// The font pattern matches by attribute lookahead because Vite rewrites the source
// preload tag and does not preserve its attribute order.
const CRITICAL_ASSET_PATTERNS = [
  ...
  {
    as: "font",
    label: "webfont preload",
    pattern: /<link(?=[^>]*rel="preload")(?=[^>]*as="font")[^>]*href="\.(\/assets\/[^"]+\.woff2)"/,
  },
];
