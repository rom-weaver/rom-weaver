...
describe("emulator retention policy", () => {
  it("retains supported ROMs at or below the cap", () => {
    expect(shouldRetainEmulatorOutput({ fileName: "game.nes", size: MAX_BYTES })).toBe(true);
  });
  ...
  it("skips unsupported platforms and oversized outputs", () => {
    ...
    expect(shouldRetainEmulatorOutput({ fileName: "game.nes", platform: "Nintendo Wii", size: 1 })).toBe(false);
