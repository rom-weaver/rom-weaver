...
<p>Open the <a href="/apply-patch?bundle=first-weave.zip">practice run</a> on ${PRODUCTION}.</p>
...
describe("retargetSampleUrls", () => {
  ...
  it("leaves prose and links on the production site", () => {
    expect(retargetSampleUrls(GUIDE, PREVIEW)).toContain(
      `<a href="/apply-patch?bundle=first-weave.zip">practice run</a> on ${PRODUCTION}.`,
    );
  });
