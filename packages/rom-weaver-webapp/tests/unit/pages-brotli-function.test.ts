...
const makeContext = ({
  ...
  return {
    context: {
      ...
      next: (input?: RequestInfo | URL, init?: RequestInit) => {
        if (input === undefined) return Promise.resolve(NEXT_SENTINEL);
...
describe("pages brotli sidecar function", () => {
  ...
  it("serves the minified document sidecar for a clean route", async () => {
    ...
    expect(fetchLog).toEqual([{ method: "GET", url: "https://rom-weaver.com/assets/html/routes/docs/faq/" }]);
    expect(response.headers.get("Content-Type")).toBe("text/html; charset=utf-8");
    expect(response.headers.get("Content-Encoding")).toBe("br");
    expect(response.headers.get("Content-Length")).toBe(String("brotli-bytes".length));
  ...
  it("falls back from a clean route to its extension sidecar", async () => {
    const { context, fetchLog } = makeContext({
      ...
      sidecarResponse: (url) =>
        ["/assets/html/routes/trim/"].some((suffix) => url.endsWith(suffix))
          ? new Response("missing", { status: 404 })
          : brSidecar(),
    });
    const response = await onRequest(context);
    expect(fetchLog).toEqual([
      { method: "GET", url: "https://rom-weaver.com/assets/html/routes/trim/" },
      { method: "GET", url: "https://rom-weaver.com/assets/html/files/trim/" },
    ]);
    expect(response.headers.get("Content-Encoding")).toBe("br");
  });
  ...
  it("matches a document validator without forwarding the full browser request", async () => {
    ...
    expect(response.headers.get("Content-Encoding")).toBe("br");
    expect(fetchLog).toEqual([{ method: "GET", url: "https://rom-weaver.com/assets/html/routes/apply/" }]);
