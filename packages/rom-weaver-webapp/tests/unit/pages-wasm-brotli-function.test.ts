...
describe("pages wasm brotli function", () => {
  it("serves the sidecar bytes with Content-Encoding br for br-capable clients", async () => {
  ...
  it("ignores non-wasm asset paths without touching ASSETS", async () => {
    const { context, fetchedUrls } = makeContext({
      url: "https://rom-weaver.com/assets/index-DNizHHMa.js",
    });
    expect(await onRequestGet(context)).toBe(NEXT_SENTINEL);
