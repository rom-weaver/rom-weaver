...
const shell = (serviceWorkerStatus, betaToolsEnabled = false, offlineProgress = null) =>
  createElement(
...
test("hydrates parser-resolved runtime nodes in place", async () => {
  const host = document.createElement("div");
  host.innerHTML = renderToString(shell(null));
  document.body.append(host);

  // The parser-time resolver writes the runtime state, title, glyph, and label.
  ...
  await act(async () => {
    root = hydrateRoot(host, shell("off"), {
...
test("hydrates the beta navigation in place when the persisted flag is enabled", async () => {
  ...
  const recoverableErrors = [];
  await act(async () => {
    root = hydrateRoot(host, shell("off", true), {
      onRecoverableError: (error) => recoverableErrors.push(error),
    });
  });
